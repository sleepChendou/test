<template>
	<el-upload action="#"  :on-remove="handleRemove" ref="upload"
	      :on-preview="handlePictureCardPreview"  list-type="picture-card" 
		  :auto-upload="false" :on-change="handleUpload" :before-remove="beforeRemove"
		  :multiple="true">
		<template #default>
		...
		methods: {
			...
			submitUpload() {
				...
				this.axios({
					...
					this.$router.push({path:'/imgs',query:{list:response.data.data.list}})
				}).catch(function(err) {
					console.log("上传失败")
					console.log(err)
				})
